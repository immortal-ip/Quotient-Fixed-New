...
class PremiumCog(Cog, name="Premium"):
    def __init__(self, bot: Quotient):
        ...
        self.hook = (
            discord.Webhook.from_url(self.bot.config.PUBLIC_LOG, session=self.bot.session)
            if getattr(self.bot.config, "PUBLIC_LOG", None)
            else None
        )
    ...
    async def pstatus(self, ctx: Context):
        """Get your Quo x Premium status and the current server's."""
        ...
        embed = self.bot.embed(ctx, title="Quo x Premium", url=f"{self.bot.config.WEBSITE}")
    ...
    async def premium(self, ctx: Context):
        """Checkout Quo x Premium Plans."""
        _e = discord.Embed(
            color=self.bot.color,
            description=f"[**Features of Quo x Pro -**]({self.bot.config.SERVER_LINK})\n\n"
            f"{emote.check} Access to `Quo x Pro` bot.\n"
    ...
    async def on_guild_premium_timer_complete(self, timer: Timer):
        ...
        if (_ch := _g.private_ch) and _ch.permissions_for(_ch.guild.me).embed_links:
            _e = discord.Embed(
                color=discord.Color.red(), title="⚠️__**Quo x Pro Subscription Ended**__⚠️", url=config.SERVER_LINK
            )
            _e.description = (
                "This is to inform you that your subscription of Quo x Pro has been ended.\n\n"
    ...
    async def on_premium_purchase(self, txnId: str):
        ...
        with suppress(discord.HTTPException, AttributeError):
            _e = discord.Embed(
                color=discord.Color.gold(), description=f"Thanks **{member}** for purchasing Quo x Premium."
            )
        ...
        _e = discord.Embed(
            color=self.bot.color,
            title="Quo x Pro Purchase Successful!",
            url=self.bot.config.SERVER_LINK,
            description=(
                f"{random_greeting()} {member.mention},\n"
                f"Thanks for purchasing Quo x Premium. Your server **{upgraded_guild}** has access to Quo x Pro features until `{_guild.premium_end_time.strftime('%d-%b-%Y %I:%M %p')}`.\n\n"
                "[Click me to Invite Quo x Pro Bot to your server](https://discord.com/oauth2/authorize?client_id=902856923311919104&scope=applications.commands%20bot&permissions=21175985838)\n"
            ),
        )

        if member not in self.bot.server.members:
            _e.description += f"\n\n[To claim your Premium Role, Join Quo x HQ]({self.bot.config.SERVER_LINK})."
