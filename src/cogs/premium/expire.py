...
async def remind_guild_to_pay(guild: discord.Guild, model: Guild):
    if (_ch := model.private_ch) and _ch.permissions_for(_ch.guild.me).embed_links:
        _e = discord.Embed(
            ...
            title="⚠️__**Quo x Pro Ending Soon**__⚠️",
            url=config.SERVER_LINK,
        )

        _e.description = (
            f"This is to inform you that your subscription of **Quo x Pro** is ending soon "
            f"({discord_timestamp(model.premium_end_time,'D')})"
            "\n\n*Kindly renew your subscription to continue using Quo x Premium features.*"
        )
        ...
        _view = PremiumView(label="Renew Quo x Pro")
        await _ch.send(
            embed=_e,
            view=_view,
...
async def remind_user_to_pay(user: discord.User, model: User):
    ...
    _e.description = (
        f"This is to remind you that your subscription of **Quo x Pro** is ending {discord_timestamp(model.premium_expire_time)}"
        f"\n[*Click Me To Continue Enjoying Quo x Pro*](https://quotientbot.xyz/premium)"
    )
