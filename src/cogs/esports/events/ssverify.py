...
class Ssverification(Cog):
    def __init__(self, bot: Quotient):
        ...
        _fastapi_url = getattr(self.bot.config, "FASTAPI_URL", "") or ""
        _fastapi_key = getattr(self.bot.config, "FASTAPI_KEY", "") or ""
        self.request_url = _fastapi_url.rstrip("/") if _fastapi_url else ""
        self.headers = {
            "Content-Type": "application/json",
        }
        self._ocr_enabled = bool(_fastapi_url and _fastapi_key)
    ...
    async def on_message(self, message: discord.Message):
        ...
        with suppress(discord.HTTPException):
            ...
            if not self._ocr_enabled:
                ...
                _ocr = []
                for attachment in attachments:
                    _ocr.append(await self.__ocr_image(message, attachment))
    ...
    async def __ocr_image(self, message: discord.Message, attachment: discord.Attachment) -> ImageResponse:
        apikey = getattr(self.bot.config, "FASTAPI_KEY", "") or ""
        url = attachment.proxy_url
        ...
        try:
            async with self.bot.session.post(
                self.request_url,
                data={"apikey": apikey, "url": url, "language": "eng", "isOverlayRequired": False},
            ) as resp:
                if resp.status == 200:
                    try:
        ...
        if status == "failed":
            try:
                ...
                data.add_field("apikey", apikey)
                data.add_field("language", "eng")
                data.add_field("isOverlayRequired", "false")
                data.add_field("file", io.BytesIO(img_bytes), filename=attachment.filename or "screenshot.png")

                async with self.bot.session.post(self.request_url, data=data) as resp:
                    if resp.status == 200:
                        try:
                            result = await resp.json()
                            if isinstance(result, dict) and result.get("ParsedResults"):
        ...
        return ImageResponse(
            ...
            phash="",
            status=status,
            text=parsed or "No text detected",
            platform="any",
        )

    async def __verify_screenshots(self, ctx: Context, record: SSVerify, _ocr: List[ImageResponse]) -> discord.Embed:
        _e = discord.Embed(color=discord.Color.yellow(), description="")

        for _ in _ocr:
            if not record.allow_same and _.dhash and _.phash:
    ...
    def __valid_attachments(self, message: discord.Message):
        ...
        return [
            ...
            if a.content_type in allowed
            or (a.filename and a.filename.lower().endswith((".png", ".jpg", ".jpeg", ".webp")))
        ]
