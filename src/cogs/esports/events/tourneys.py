...
class TourneyEvents(Cog):
    ...
    async def on_guild_channel_update(self, before: discord.TextChannel, after: discord.TextChannel):
        if before.name == after.name or not before.name == "quox-tourney-logs":
            return

        if after.permissions_for(after.guild.me).manage_channels:
            return await after.edit(
                name="quox-tourney-logs",
        ...
        _e = discord.Embed(
            color=discord.Color.red(),
            description=(
                "Someone renamed this channel kindly rename it back to `quox-tourney-logs`, "
                "**Quo x Tourneys won't work without it.**"
            ),
        )
        await after.send(
    ...
    async def on_guild_role_update(self, before: discord.Role, after: discord.Role):
        ...
        _e = discord.Embed(
            color=discord.Color.red(),
            description=(
                f"Someone renamed Quo x's tourney-mod role to {after.mention}, kindly rename it back to `tourney-mod`."
                "**Quo x Tourneys need the name of this role to be `tourney-mod`**"
            ),
        )

        c = discord.utils.get(after.guild.text_channels, name="quox-tourney-logs")
        if c:
            await c.send(
                embed=_e, content=getattr(after.guild.owner, "mention")
            )  # there is very less chances of getting attribute error on `owner.mention`
