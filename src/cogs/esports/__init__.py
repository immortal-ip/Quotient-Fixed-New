...
from cogs.esports.views.tourney.modal_reg import TourneyRegistrationPanel
from models import *
...
class ScrimManager(Cog, name="Esports"):
    ...
    async def smanager(self, ctx: Context):
        """
        Contains commands related to Quo x's powerful scrims manager.
        """
    ...
    async def tourney(self, ctx: Context):
        """Create & Manage tournaments with Quo x"""
    ...
    async def easytag(self, ctx: Context):
        """Commands related to quox's eztag"""
    ...
    async def set_eztag(self, ctx: Context, *, channel: QuoTextChannel):
        ...
        role = discord.utils.get(ctx.guild.roles, name="quox-tag-ignore")
        if not role:
            role = await ctx.guild.create_role(
                name="quox-tag-ignore", color=self.bot.color, reason=f"Created by {ctx.author}"
            )
        ...
        embed.description = """
        Unable to mention teammates while registering for scrims or tournaments? Quo x is here for the rescue.

        Use `teammate's ID`, `@teammate_name` or `@teammate's_discord_tag` in your registration format. Quo x will convert that into an actual discord tag.        
        """
    ...
    @commands.has_permissions(manage_guild=True)
    async def tourney_modal_panel(self, ctx: Context, tourney_id: int):
        """Post a modal-based registration panel for a tournament. Usage: Btpanel <tourney_id>"""
        from models import Tourney
        tourney = await Tourney.get_or_none(pk=tourney_id, guild_id=ctx.guild.id)
        if not tourney:
            return await ctx.error(f"No tournament found with ID `{tourney_id}`.\nUse `{ctx.prefix}tourney config` to find the ID.")

        view = TourneyRegistrationPanel(tourney_id)
        ctx.bot.add_view(view)

        _e = discord.Embed(color=ctx.bot.color)
        _e.description = (
            f"**{tourney.name}**\n\n"
            f"Tap the button below to register your team and join the action!"
        )
        _e.set_thumbnail(url=getattr(ctx.guild.icon, "url", ctx.bot.user.avatar.url))

        await ctx.send(embed=_e, view=view)
        try:
