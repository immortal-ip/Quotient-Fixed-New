...
class Utility(Cog, name="utility"):
    ...
    async def on_message(self, message: discord.Message):
        ...
        if message.author.id in self.afk_users:
            ...
            try:
                await message.channel.send(f"Welcome back **{message.author}**! I removed your AFK status.")
        ...
        for mention in message.mentions:
            if mention.id in self.afk_users and mention.id != message.author.id:
                ...
                time_str = f"<t:{int(timestamp.timestamp())}:R>" if timestamp else ""
    ...
    async def autopurge(self, ctx: Context):
        """
        Set Quo x to delete every new message in a channel after  a specific interval.
        """
        await ctx.send_help(ctx.command)
    ...
    @commands.command()
    async def avatar(self, ctx: Context, *, member: typing.Optional[QuoMember]):
        """Show user avatar and server avatar."""
        member = member or ctx.author

        embed = self.bot.embed(ctx, title=f"{member.display_name}'s Avatar")
        embed.set_image(url=member.display_avatar.url)

        if member.guild_avatar:
            embed.add_field(name="Server Avatar", value=f"[Click Here]({member.guild_avatar.url})", inline=False)

        embed.add_field(name="Global Avatar", value=f"[Click Here]({member.display_avatar.url})", inline=False)
        await ctx.send(embed=embed)

    @commands.command()
    async def banner(self, ctx: Context, *, member: typing.Optional[QuoMember]):
        """Show user banner and server banner."""
        member = member or ctx.author

        user = await self.bot.fetch_user(member.id)
        embed = self.bot.embed(ctx, title=f"{member.display_name}'s Banner")
        ...
        if user.banner:
            ...
            if ctx.guild.banner:
                ...
                await ctx.error("This user and the server have no banners.")

    @commands.command()
    async def afk(self, ctx: Context, *, reason: str = "No reason provided"):
        """Set your AFK status with a custom reason."""
        if ctx.author.id in self.afk_users:
            return await ctx.error("You are already AFK. Use `+afk` again to update your reason.")

        self.afk_users[ctx.author.id] = {
            "reason": reason,
            "timestamp": discord.utils.utcnow(),
        }
        await ctx.success(f"You are now AFK: {reason}")
