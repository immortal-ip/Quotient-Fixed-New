...
class CmdEvents(Cog):
    ...
    async def bot_check(self, ctx: Context):
        ...
        if self.bot.lockdown is True:
            t = (
                "**Quo x is getting new features** 🥳\n"
                "Dear user, Quo x is updating and is not accepting any commands.\n"
        ...
        with suppress(discord.HTTPException):
            ...
            if not member.bot and record.humans:
                for role in record.humans:
                    try:
                        await member.add_roles(discord.Object(id=role), reason="Quo x's autorole")
