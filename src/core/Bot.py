...
class Quotient(commands.AutoShardedBot):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(
            ...
            activity=discord.Activity(type=discord.ActivityType.listening, name="Scrims & Tourney Manager"),
        ...
        self.message_cache: Dict[int, Any] = LRUCache(maxsize=1024)
    ...
    async def setup_hook(self) -> None:
        ...
        for coro_func in on_startup:
            asyncio.get_event_loop().create_task(coro_func(self))
    ...
    async def on_command(self, ctx: Context):
        ...
        await self.db.execute(
            "INSERT INTO user_data (user_id, made_premium) VALUES ($1, $2) ON CONFLICT DO NOTHING",
            ctx.author.id,
            [],
        )
    ...
    def server(self) -> Optional[discord.Guild]:
        return self.get_guild(getattr(self.config, "SERVER_ID", 0))
...
async def bot_before_invoke(ctx: Context):
    if ctx.guild is not None and not ctx.guild.chunked:
        asyncio.get_event_loop().create_task(ctx.guild.chunk())
