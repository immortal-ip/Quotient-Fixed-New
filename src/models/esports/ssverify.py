...
class SSVerify(BaseDbModel):
    ...
    async def verify_yt(self, ctx: Context, image: ImageResponse):
        ...
        _yt_text = image.text.upper().replace("\n", " ")
        if "SUBSCRIBE " in _yt_text and "SUBSCRIBED" not in _yt_text:
    ...
    async def verify_insta(self, ctx: Context, image: ImageResponse):
        if not any(_ in image.lower_text for _ in ("followers", "follower", "following", "follow", "posts")):
